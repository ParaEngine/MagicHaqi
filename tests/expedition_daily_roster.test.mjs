...
test('每日远征后段才解锁 SSR 与 UR，且不会按层数稳定产出高品质伙伴', () => {
    const roster = getDailyExpeditionRoster({}, { now: new Date(2026, 7, 14, 10, 0, 0) });

    for (const expedition of roster) {
        const enemies = expedition.nodes.filter(node => node.enemy);
        assert.ok(enemies.filter(node => node.level <= 10).every(node => !['史诗', '传说'].includes(node.enemy.rarity)));
        assert.ok(enemies.filter(node => node.level <= 22).every(node => node.enemy.rarity !== '传说'));
        assert.equal(enemies.at(-1).enemy.rarity, '传说');
    }
});