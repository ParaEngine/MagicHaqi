...
import { hasMineralBridgeSyncChanges, mergeMineralBridgeSync, settleMineralRoutePreparation } from '../js/mineral_host_core.js';
...
test('矿区普通同步不能覆盖宿主经济与防重放账本', () => {
    const current = {
        dispatchedPetIds: ['pet-1'],
        activeSeriesIds: ['old-series'],
        bonuses: { attackPercent: 2, expeditionLootPercent: 3 },
        research: 9,
        preparationCharges: 2,
        tacticalItems: { emergencyBeacon: 1, deflectionShield: 1 },
        breedingCatalysts: { ssrMutation: 1, urAttributeLock: 1 },
        workshop: { day: '2026-08-28', refined: 4, soldCoins: 12 },
        consumedRequestIds: ['used-1'],
    };
    const result = mergeMineralBridgeSync({
        current,
        knownPetIds: new Set(['pet-2']),
        payload: {
            dispatchedPetIds: ['pet-2', 'unknown'],
            activeSeriesIds: ['new-series'],
            bonuses: { attackPercent: 12, expeditionLootPercent: 14 },
            research: 0,
            preparationCharges: 0,
            tacticalItems: {},
            breedingCatalysts: {},
            workshop: { day: '', refined: 0, soldCoins: 0 },
            consumedRequestIds: [],
        },
    });

    assert.deepEqual(result.dispatchedPetIds, ['pet-2']);
    assert.deepEqual(result.activeSeriesIds, ['new-series']);
    assert.deepEqual(result.bonuses, { attackPercent: 12, expeditionLootPercent: 14 });
    assert.equal(result.research, 9);
    assert.equal(result.preparationCharges, 2);
    assert.deepEqual(result.tacticalItems, current.tacticalItems);
    assert.deepEqual(result.breedingCatalysts, current.breedingCatalysts);
    assert.deepEqual(result.workshop, current.workshop);
    assert.deepEqual(result.consumedRequestIds, ['used-1']);
});

test('矿区相同快照不触发宿主整页重渲染', () => {
    const current = {
        dispatchedPetIds: ['pet-1'],
        activeSeriesIds: ['series-1'],
        bonuses: { attackPercent: 6, expeditionLootPercent: 14 },
        syncedAt: 1,
    };
    const unchanged = mergeMineralBridgeSync({
        current,
        knownPetIds: new Set(['pet-1']),
        payload: {
            dispatchedPetIds: ['pet-1'],
            activeSeriesIds: ['series-1'],
            bonuses: { attackPercent: 6, expeditionLootPercent: 14 },
    ...
    const changed = mergeMineralBridgeSync({
        current,
        knownPetIds: new Set(['pet-1', 'pet-2']),
        payload: { dispatchedPetIds: ['pet-2'] },
    });

    assert.equal(hasMineralBridgeSyncChanges(current, unchanged), false);
    assert.equal(hasMineralBridgeSyncChanges(current, changed), true);
});
