        ...
        <div class="absolute" style="top:52px;left:0;right:0;bottom:0;overflow-y:auto;padding:14px">
            <div class="card-flat fade-in" style="display:flex;gap:14px;align-items:center;margin-bottom:12px">
                ...
                <div style="flex:1;min-width:0">
                    <div class="text-lg font-extrabold" style="color:var(--text-primary)">${escapeHtml(displayPetName(pet))}${isAdultStage(pet.stage) ? '' : ` <span style='font-size:11px;color:var(--text-muted);font-weight:600'>${escapeHtml(t('nameUnknownSub'))}</span>`}</div>
                    ...
                    <div class="text-xs mt-1" style="color:var(--text-muted)">${escapeHtml(t('rarity'))}：<b style="color:var(--accent-dark)">${pet.rarity ?? '?'}</b></div>
                </div>
            </div>

            <div class="card-flat mb-3">
                <div class="text-xs font-bold mb-2" style="color:var(--text-secondary)">${escapeHtml(t('bodyState'))}</div>
                ${sickness ? `
            ...
            <div class="card-flat mb-3">
                ...
                ` : `
                    ...
                    <div class="text-xs mt-2" style="color:var(--text-muted);font-style:italic">${escapeHtml(t('traitsHiddenHint'))}</div>
                `) : ''}
                ...
                <div class="text-xs font-bold mt-3 mb-1" style="color:var(--text-secondary)">${escapeHtml(t('growthTraits'))}</div>
                ...
                ` : `<div class="text-xs" style="color:var(--text-muted)">${escapeHtml(t('noGrowthTraits'))}</div>`}
            </div>
