...
const ENEMIES_BY_RARITY = Object.freeze({
    普通: ENEMIES.filter(enemy => enemy.rarity === '普通'),
    稀有: ENEMIES.filter(enemy => enemy.rarity === '稀有'),
    精英: ENEMIES.filter(enemy => enemy.rarity === '精英'),
    史诗: ENEMIES.filter(enemy => enemy.rarity === '史诗'),
    传说: ENEMIES.filter(enemy => enemy.rarity === '传说'),
});
const ENCOUNTER_WEIGHTS_BY_DEPTH = Object.freeze([
    { maxLevel: 10, weights: Object.freeze({ 普通: 0.78, 稀有: 0.21, 精英: 0.01 }) },
    { maxLevel: 15, weights: Object.freeze({ 普通: 0.72, 稀有: 0.25, 精英: 0.028, 史诗: 0.002 }) },
    { maxLevel: 22, weights: Object.freeze({ 普通: 0.70, 稀有: 0.25, 精英: 0.045, 史诗: 0.005 }) },
    { maxLevel: 27, weights: Object.freeze({ 普通: 0.65, 稀有: 0.27, 精英: 0.07, 史诗: 0.009, 传说: 0.001 }) },
    { maxLevel: 29, weights: Object.freeze({ 普通: 0.58, 稀有: 0.29, 精英: 0.11, 史诗: 0.018, 传说: 0.002 }) },
]);
...
function enemyForLevel(level, random) {
    const weights = ENCOUNTER_WEIGHTS_BY_DEPTH.find(entry => level <= entry.maxLevel)?.weights
        || ENCOUNTER_WEIGHTS_BY_DEPTH.at(-1).weights;
    let roll = random();
    for (const [rarity, weight] of Object.entries(weights)) {
        roll -= weight;
        if (roll <= 0) return pick(ENEMIES_BY_RARITY[rarity], random);
    }
    return pick(ENEMIES_BY_RARITY.普通, random);
}
...
function buildNodes(seed) {
    ...
    return Array.from({ length: 30 }, (_, index) => {
        ...
        const enemy = isBoss ? ENEMIES_BY_RARITY.传说[0] : enemyForLevel(level, random);
