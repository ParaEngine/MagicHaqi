...
export function getHaqiWeeklyProgress({ history = [], bridge = {}, now = Date.now() } = {}) {
    ...
    const goals = [
        { id: 'weekly-expeditions', label: '完成 2 次成功远征', current: Math.min(2, completedExpeditions), target: 2 },
        { id: `weekly-theme-${theme.id}`, label: `完成 2 次「${theme.label}」生态远征`, hint: `目标星球：${theme.biomes.join(' / ')}`, current: Math.min(2, themeExpeditions), target: 2, themeId: theme.id, themeLabel: theme.label },
    ...
    return {
        ...
        completed: goals.filter((goal) => goal.complete).length,
        total: goals.length,
        goals,
    };
}
