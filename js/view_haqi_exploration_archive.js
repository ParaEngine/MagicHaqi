...
import { aggregateExpeditionPlaytests } from './expedition_history.js';
...
function formatHistory(entry) {
    ...
    const materialText = completed
        ...
        : '未带回战利品';
    const supportText = Array.isArray(playtest?.supports) && playtest.supports.length
        ? playtest.supports.map(item => item.specialtyId || item.name).filter(Boolean).join(' + ')
        : '无支援';
    const mutationText = playtest?.mutationChoice === 'accepted'
        ? '接受异变'
        : playtest?.mutationChoice === 'declined' ? '拒绝异变' : '';
    const playtestText = playtest
        ? [supportText, playtest.routePlanId, playtest.investigation?.branchId, mutationText].filter(Boolean).join(' · ')
        : '';
    return {
        completed,
        title: `${completed ? '完成' : '失败'} · ${entry?.expeditionName || entry?.expeditionId || '未知星图'}`,
        detail: `${entry?.petName || '出战伙伴'} · ${materialText}${playtestText ? `\n${playtestText}` : ''}`,
    };
}

function formatSectorEvent(progress) {
    const stage = String(progress?.stage || 'dormant');
    const branchNames = {
        'missing-companion': '追踪失踪伙伴',
        'sunken-ruins': '解读地下遗迹',
    };
    const stages = {
        dormant: { chapter: '未开启', title: '星尘潮汐', next: '前往荧光沼泽，发现第一条孢子异变线索。' },
        'week1-discovery': { chapter: '第一章', title: '异常扩散', next: '返回星图选择主要调查方向。' },
        'week2-divergence': { chapter: '第二章', title: '分路调查', next: '沿选定路线收集四条调查证据。' },
        'week3-convergence': { chapter: '第三章', title: '源头汇合', next: '进入异变源头，完成关键调查。' },
        'week4-finale': { chapter: '第四章', title: '共同终局', next: '终局准备已冻结，等待星尘潮汐决战开放。' },
        archived: { chapter: '已归档', title: '潮汐余响', next: progress?.resolution?.ecologyChange || '本期星域事件已完成。' },
    };
    const current = stages[stage] || stages.dormant;
    return {
        ...current,
        branch: branchNames[progress?.primaryBranchId] || '尚未选择调查方向',
    };
}
...
export function renderHaqiExplorationArchive(panel, data = {}, { onBack, onOpenExpedition, onOpenMineral, onClaimWeeklyReward } = {}) {
    ensureStyles();
    const bridge = data.bridge || {};
    const bonuses = bridge.bonuses || {};
    const history = Array.isArray(data.history) ? data.history.slice(0, 5) : [];
    const weeklyProgress = data.weeklyProgress || getHaqiWeeklyProgress({ history: data.history, bridge });
    ...
    const collectibleSeries = Array.isArray(data.collectibleSeries) ? data.collectibleSeries : [];
    ...
    const sectorEvent = formatSectorEvent(data.sectorEventProgress);
    const playtestStats = aggregateExpeditionPlaytests(data.history);
    const completionPercent = Math.round(playtestStats.completionRate * 100);
    const mutationPercent = Math.round(playtestStats.mutationAcceptanceRate * 100);
        ...
        <main class="haqi-archive">
            ...
            <div class="haqi-archive__body">
                ...
                <section class="haqi-archive__section" style="margin-top:12px">
                    <h2>星域事件 · ${escapeHtml(sectorEvent.chapter)}</h2>
                    <p><strong>${escapeHtml(sectorEvent.title)}</strong> · ${escapeHtml(sectorEvent.branch)}<br>${escapeHtml(sectorEvent.next)}</p>
                </section>
                <section class="haqi-archive__section" style="margin-top:12px">
                    <h2>定向试玩 · 真实记录 ${playtestStats.sampleSize} 局</h2>
                    <p>完成率 ${completionPercent}% · 异变接受 ${playtestStats.mutationAcceptedCount}/${playtestStats.mutationEncounterCount || 0}（${mutationPercent}%）· 跨局重开 ${playtestStats.restartCount} 次<br>航段：${escapeHtml(formatCounts(playtestStats.routePlanCounts))}<br>未完成节点：${escapeHtml(formatCounts(playtestStats.extractionNodeCounts))}${playtestStats.sampleSize < 10 ? '<br>样本未满 10 局，仅展示观察值，暂不锁定构筑数值。' : ''}</p>
                </section>
                <section class="haqi-archive__section" style="margin-top:12px" id="mhCollectibleSeries">
                    <h2>旅途收藏系列</h2>
                    <p>远征、挖矿与小镇探索带回的藏品会按系列长期保留。</p>
                    <div class="haqi-archive__list">${collectibleSeries.map(series => `<div class="haqi-archive__row haqi-archive__route"><div class="haqi-archive__thumb" aria-hidden="true">${escapeHtml(series.icon)}</div><div class="haqi-archive__route-body"><strong>${escapeHtml(series.name)}系列</strong></div><span class="haqi-archive__route-state">${series.completed ? '已集齐' : `${nonNegative(series.currentCount)}/${nonNegative(series.totalCount)}`}</span><div class="haqi-archive__progress" style="--progress:${Math.min(100, nonNegative(series.currentCount) / Math.max(1, nonNegative(series.totalCount)) * 100)}%"><i></i></div></div>`).join('')}</div>
                </section>
                <section class="haqi-archive__section" style="margin-top:12px">
                    <h2>本周航线</h2>
                    <p>${escapeHtml(`${weekStart} 起 · 已完成 ${weeklyProgress.completed}/${weeklyProgress.total} 项。全部完成可领取 ${weeklyProgress.rewardCoins} 金币。`)}</p>
                    ...
                    <div class="haqi-archive__actions"><button class="haqi-archive__action" id="mhHaqiArchiveWeeklyClaim" type="button" ${weeklyProgress.claimable ? '' : 'disabled'}>${weeklyProgress.claimed ? '本周奖励已领取' : weeklyProgress.claimable ? `领取 ${weeklyProgress.rewardCoins} 金币` : '完成全部目标后领取'}</button></div>
                </section>
    ...
    panel.querySelector('#mhHaqiArchiveWeeklyClaim').onclick = () => onClaimWeeklyReward?.();
}
